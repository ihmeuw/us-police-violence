...
suppressMessages(library(lbfgs))
suppressMessages(library(GauPro))

root <- paste0("FIELPATH")
...
young <- data.table(age_group_id=28, age_group_name="0 to 1", age_group_years_start=0, age_group_years_end=1)
age_info <- rbind(age_info, young)
...
# Get GBD population information
...
# Use National race-specific estimates, rather than state information
files <- c("FILEPATH", "FILEPATH")
dt.nat <- list()
for (file in files){
  nat <- data.table(read_sas(paste0(scratch, "FILEPATH/", file)))
...
load(paste0(scratch, "FILEPATH"))

# Load raw population data for NCHS bridged-race files
# 1990-1999
one <- fread(paste0(scratch, "FILEPATH"), fill=T, colClasses="character")
...
# 2000-2004
two <- fread(paste0(scratch, "FILEPATH"), fill=T, colClasses="character")
...
# 2005-2009
three <- fread(paste0(scratch, "FILEPATH"), fill=T, colClasses="character")
...
# 2010-2019
four <- data.table(read_sas(paste0(scratch, "FILEPATH")))
...
#Load in 1980 census data
ddi <- read_ipums_ddi(paste0(scratch, "FILEPATH"))
...
#Split oldest and youngest age groups
...
young <- merge(dt.combined[age==0, .(location_id, sex_id, year_id, age, race, parent_id, pop)],
...
young[,population:=pop*state_prop]
...
dt.split <- rbind(young, dt.split, old, fill=T)
...
saveRDS(pop.final, file=paste0(scratch, "FILEPATH"))
...
saveRDS(pop.scaled, file=paste0(scratch, "FILEPATH"))
...
pdf(file=paste0(plot_dir, "FILEPATH"), width=14, height=8.5)
for (d in detailed){
...
# Pull population information for global/super region/region
...
write.csv(pop.upload, file=paste0(root, "FILEPATH"), row.names=F)
...
pdf(file=paste0(plot_dir, "FILEPATH"), width=17, height=8.5)
for (i in 1:length(state_names)){
  df.graph <- subset(pop.scaled, location_name==state_names[i])
  df.graph[,sex_name := ifelse(sex_id==1, "Male", "Female")]
  df.graph[,age_group := factor(age_group_id, levels=c(2,3,388,389,238,34,6:20,30,31,32,235))]
...
for (i in 1:length(state_names)){
  ...
  df.graph[,sex_name := ifelse(sex_id==1, "Male", "Female")]
  p <- ggplot(data=df.graph, aes(x=pop.original, y=pop.scaled,
...
pdf(paste0(plot_dir, "FILEPATH"), width=12, height=7)
